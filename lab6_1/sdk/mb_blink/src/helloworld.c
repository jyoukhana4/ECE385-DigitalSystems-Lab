...
volatile uint32_t* led_gpio_data = 0x40000000;               //Hint: either find the manual address (via the memory map in the block diagram, or
															 //replace with the proper define in xparameters (part of the BSP). Either way//this is the base address of the GPIO corresponding to your LEDs
															 //(similar to 0xFFFF from MEM2IO from previous labs).
volatile uint32_t* switches_gpio_data = 0x40010000;
volatile uint32_t* buttons_gpio_data = 0x40020000;
...
int main()
{
	...
	{
		/*
		if (*buttons_gpio_data == 0x1) {
			*led_gpio_data = 0;
		} */
		if (*buttons_gpio_data == 0x1) {
			while (*buttons_gpio_data == 0x1) {
				continue;
			}
			if (*led_gpio_data + *switches_gpio_data > 65535) {
				*led_gpio_data = 0;
				printf("Overflow\r\n");
			}
			else {
				*led_gpio_data += *switches_gpio_data;
			}
		}
		/*
		sleep(1);
